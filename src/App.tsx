...
import { StructuredTicket, ChatMessage, TaskType, UrgencyLevel, ClinicalImpact, TaskStatus, LLMConfig, UserProfile } from './types';
import { INITIAL_TASKS } from './data/defaultTasks';
import TaskStats from './components/TaskStats';
import EquipmentArchives from './components/EquipmentArchives';

// Quick Preset Prompts for clinical staff
...
const DEFAULT_LLM_PRESETS: LLMConfig[] = [
  {
    id: 'gemini-default',
...
故障现象: 设备异常，需装备科紧急派人进行维修或PM检查。`;
  ...
  const fallbackParse = (text: string): Partial<StructuredTicket> => {
    ...
    // 4. Device Name
    let deviceName = '';
    const devMatch = text.match(/(呼吸机|除颤仪|麻醉机|监护仪|氧气|负压吸引|胃镜|dr|电脑|打印机|注射泵|输液泵)/i);
    if (devMatch) {
